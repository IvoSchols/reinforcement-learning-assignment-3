...
class ActorCriticBootstrapAgent:
    ...
    def compute_returns(self, traces):
        returns = []
        for t in range(len(traces) - self.n_steps + 1):
            n_step_return = 0
    ...
    def update_policy(self, traces, returns):
        actor_losses = []
        critic_losses = []
        entropy_losses = []
    ...
    def train(self, num_episodes, num_traces):
        ...
        for episode in range(num_episodes):
            ...
            returns = self.compute_returns(traces)
            self.update_policy(traces, returns)
...
if __name__ == '__main__':
    ...
    agent = ActorCriticBootstrapAgent(env, device)
